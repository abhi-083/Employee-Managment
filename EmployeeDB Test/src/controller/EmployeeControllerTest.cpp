...
TEST_F(EmployeeFixture, Test_insertEmployee) {
	...
	std::string_view queryString = R"(SELECT * FROM Employee WHERE firstName = "Abhi" COLLATE NOCASE;)";
...
TEST_F(EmployeeFixture, Test_updateEmployee) {
	...
	queryStr = R"(SELECT * FROM Employee WHERE lastName ="Patel")";
...
TEST_F(EmployeeFixture, Test_getUpdateQueryCondition) {
	EXPECT_EQ(EmployeeControllerTest::getUpdateQueryCondition(*employee), R"(firstName = "Abhi", middleName = "Narendrakumar", lastName = "Patel", dateOfBirth = "30-04-2003", mobileNo = 7283902430, email = "abhi.patel@zuru.com", address = "82, V.P.", gender = "Male", dateOfJoining = "01-01-2024", mentorID = 1, performanceMetric = 5.000000, bonus = 100000.000000)");
